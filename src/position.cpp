...
bool Position::CanCastle(COLOR side, U8 flank) const
{
    ...
    for (auto f : flank == KINGSIDE ? m_kingSideCastling[side] : m_queenSideCastling[side]) {
...
void Position::Clear()
{
    ...
    std::memset(move_o_o, 0, sizeof(move_o_o));
    std::memset(move_o_o_o, 0, sizeof(move_o_o_o));
    m_ep = NF;
    m_fifty = 0;
    m_hash = 0;
